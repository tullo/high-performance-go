...
# The parameters to a variadic function are boxed into a slice,
# in this case a []interface{}
...
- Does this optimisation hold true if count is a variable, not a constant?
    ...
    const count = 100
    go build -gcflags=-m examples/esc/sum.go
    > examples/esc/sum.go:7:17: make([]int, count) does not escape
    ```
